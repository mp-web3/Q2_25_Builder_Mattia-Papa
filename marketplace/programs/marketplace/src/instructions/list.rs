...
pub struct List<'info> {
    ...
    #[account(
        ...
        bump,
        constraint = metadata.collection.as_ref().unwrap().key().as_ref() == collection_mint,
...
impl<'info> List <'info> {
    pub fn create_listing(&mut self, price: u64, bumps: &ListBumps) -> Result<()> {
        self.listing.set_inner(Listing {maker: self.maker.key(), maker_mint: self.maker_mint.key(), price, bump: bumps.listing});
    ...
    pub fn deposite_nft(&mut self) -> Result<()> {
        ...
        let cpi_ctx: CpiContext<'_,> = CpiContext::new(cpi_program, cpi_accounts);
